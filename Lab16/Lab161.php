...
<BODY>
<H1>Consulta de Servicio Web de Conversión de Temperatura</H1> <FORM NAME="FormConv" METHOD="post" ACTION="lab161.php">
<BR/>
Convertir desde <SELECT NAME="temp"> <OPTION value="CtoF" SELECTED>°C a °F
<OPTION value="FtoC">°F a °C
</SELECT> el valor
<input TYPE="number" NAME="valor" step="Any" required> <INPUT NAME="Convertir" VALUE="Convertir" TYPE="submit" />
</FORM>
<?PHP
$servicio="https://www.w3schools.com/xml/tempconvert.asmx?wsdl";
$parametros=array();
if (array_key_exists('Convertir', $_POST)) {
$valor=$_POST['valor']; $temp=$_POST['temp'];
if ($temp=="CtoF") {
...
$resultado = $resultObj->CelsiusToFahrenheitResult;
    ...
    $cliente = new SoapClient ($servicio, $parametros); $resultObj = $cliente->FahrenheitToCelsius ($parametros); 
